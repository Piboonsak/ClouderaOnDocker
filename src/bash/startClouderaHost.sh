...
docker start hd-c1nn
ssh docker101 docker start hd-c2nn
ssh docker102 docker start hd-c1dn1
ssh docker105 docker start hd-c1dn2
ssh docker106 docker start hd-c1dn3
ssh docker107 docker start hd-c1dn4

echo "--------Start All Hosts-------------------"
...
echo "$HOSTNAME"
docker exec -it hd-c1nn /bin/sh -c service apache2 start

echo "${C2NN_HOSTNAME}"
ssh docker101 'docker exec -i hd-c2nn /bin/sh -c service apache2 start'

echo "${C1DN1_HOSTNAME}"
ssh docker102 'docker exec -i hd-c1dn1 /bin/sh -c service apache2 start'

echo "${C1DN2_HOSTNAME}"
ssh docker105 'docker exec -i hd-c1dn2 /bin/sh -c service apache2 start'

echo "${C1DN3_HOSTNAME}"
ssh docker106 'docker exec -i hd-c1dn3 /bin/sh -c service apache2 start'

echo "${C1DN4_HOSTNAME}"
ssh docker107 'docker exec -i hd-c1dn4 /bin/sh -c service apache2 start'

echo "############# Completed generate secure keys in cluster ! ##############"

## Start service cloudera manager
docker exec -it hd-c1nn /bin/sh -c service cloudera-scm-server start
SCMSTATUS="$(docker exec -it hd-c1nn /bin/sh -c service cloudera-scm-server status)"
echo "cloudera-scm-server : ${SCMSTATUS}"

## login to hd-c1nn namenode
docker exec -it hd-c1nn bash
